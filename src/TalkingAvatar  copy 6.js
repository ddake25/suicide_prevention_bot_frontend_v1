...
const TalkingAvatar = () => {
  ...
  // HARD mute mic during TTS; restart after TTS finishes
  const speakText = (text) => {
    if (!("speechSynthesis" in window) || !text?.trim()) return;

    try {
      if (recognitionRef.current) {
        micPausedForTTSRef.current = true; // <-- tell onend/onerror not to auto-restart here
        recognitionRef.current.onresult = null;
        recognitionRef.current.onerror = null;
        recognitionRef.current.onend = null;
        recognitionRef.current.abort();   // abort => no final flush
      }
    } catch {}
    setListening(false);
    ...
    const resumeMic = () => {
      setSpeaking(false);
      // Clear TTS-mute flag BEFORE restarting STT
      micPausedForTTSRef.current = false;
      startListening(true); // keep previous transcript
    };
  ...
  const startListening = (silentStart = false) => {
    ...
    if (listening || speaking) return; // don't start while avatar is speaking

    manualPauseRef.current = false;           // user wants it running now
    clearTimeout(restartTimerRef.current);    // cancel pending restarts
    ...
    rec.onresult = async (e) => {
      if (speaking) return; // extra guard: ignore anything while avatar talks
    ...
    rec.onerror = (ev) => {
      ...
      const err = ev?.error;
      const shouldRestart =
        !manualPauseRef.current && !speaking && !micPausedForTTSRef.current && err !== "aborted";
      if (shouldRestart) {
    ...
    rec.onend = () => {
      setListening(false);
      const shouldRestart =
        !manualPauseRef.current && !speaking && !micPausedForTTSRef.current;
      if (shouldRestart) {
        clearTimeout(restartTimerRef.current);
        restartTimerRef.current = setTimeout(() => startListening(true), 150);
      }
  ...
  return (
    <Card className="shadow-sm p-4 mx-auto mt-4" style={{ maxWidth: "900px", width: "100%" }}>
      ...
      <p className="text-center text-muted mb-3">
        Start talking; each finalized phrase goes to <code>localhost:8000/chat_llm/send</code>.
