...
function VRMScene({ vrm, speaking, gestures }) {
  ...
  // Per-arm “raise” direction: +1 means increasing Z raises the hand; −1 means increasing Z lowers.
  ...
  useFrame((_, delta) => {
    ...
    if (!dirRef.current.done && lUpperArm && rUpperArm && lHand && rHand && lShoulder && rShoulder) {
      const measureRaiseSign = (upper, hand, shoulder) => {
        const saved = upper.rotation.z;
        upper.rotation.z = saved + 0.18;              // nudge +Z
        ...
        // if +Z makes hand higher than -Z, +1; else -1
        return hy1 > hy2 ? +1 : -1;
      };
      dirRef.current.l = measureRaiseSign(lUpperArm, lHand, lShoulder);
      dirRef.current.r = measureRaiseSign(rUpperArm, rHand, rShoulder);
      dirRef.current.done = true;
    }
    const raiseL = dirRef.current.l; // +1: +Z raises,  -1: +Z lowers
    ...
    const REST = 0.9;     // how close to body (bigger => lower)
    const ELBOW = 0.35;

    // If +Z raises, then REST should be -REST; if +Z lowers, REST should be +REST.
    ...
    if (lUpperArm) {
      ...
      lUpperArm.rotation.y = 0; // keep neutral
    }
    if (rUpperArm) {
    ...
    if (!(gestures && speaking)) {
      ...
      if (g.active) {
        ...
        const LIFT  = 0.45 * k;          // amount to add in “raise” direction
